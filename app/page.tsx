...
export default function Home() {
  ...
  return (
    <main className="dashboard">
      <header className="topbar"><div><p className="eyebrow">JEV TRADES / LIVE FEED</p><h1>{symbol}</h1><p className="muted">{symbol} <span className="dot" /> 1 minute candles</p></div><div className="topbar-meta"><a className="creator-link" href="https://x.com/zadescoxp" target="_blank" rel="noreferrer">made by @zade</a><div className="connection"><span className={`status-dot ${snapshot?.status === "live" ? "is-live" : ""}`} />{snapshot?.status ?? "connecting"}</div></div></header>
      ...
      <section className="contribute-panel"><div><p className="eyebrow">OPEN SOURCE</p><h2>Build with Jev Trades</h2><p className="muted">Ideas, fixes, and improvements are welcome.</p></div><a className="github-link" href="https://github.com/zadescoxp/Jev-Trades" target="_blank" rel="noreferrer">Contribute on GitHub <span aria-hidden="true">-&gt;</span></a></section>
    </main>
  );
}
