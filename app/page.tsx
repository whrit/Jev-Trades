...
export default function Home() {
  ...
  return (
    <main className="dashboard">
      ...
      <section className="portfolio-controls"><label>ASSET<select value={symbol} onChange={(event) => { const nextSymbol = event.target.value; setSymbol(nextSymbol); void configurePortfolio(false, nextSymbol); }}>{symbols.map((item) => <option key={item}>{item}</option>)}</select></label><label>CAPITAL<input type="number" min="0" step="100" value={capital} onChange={(event) => setCapital(event.target.value)} /></label><label>MAX POSITION %<input type="number" min="1" max="100" step="1" value={maxWalletPositionPct} onChange={(event) => setMaxWalletPositionPct(event.target.value)} /></label><label>RISK<select value={riskAppetite} onChange={(event) => setRiskAppetite(event.target.value)}><option value="conservative">Conservative</option><option value="balanced">Balanced</option><option value="aggressive">Aggressive</option></select></label><label className="key-field">TYPESAFE KEY<input type="password" autoComplete="off" placeholder="Optional if server has one" value={typeSafeKey} onChange={(event) => setTypeSafeKey(event.target.value)} /></label><button className="control" onClick={() => void configurePortfolio()}>Apply portfolio</button><button className={tradingEnabled ? "trade-toggle running" : "trade-toggle"} onClick={() => void configurePortfolio(!tradingEnabled)}>{tradingEnabled ? `Stop trading ${symbol}` : `Start trading ${symbol}`}</button></section>
